...
var SetExchangeFeeToSetterCmd = &cobra.Command{
	Use:     "SetExchangeFeeToSetter {from} {exchange} {feeToSetter} {password} {nonce}; Set exchange feeTo setter;",
	Aliases: []string{"setexchangefeetosetter", "sefts", "SEFTS"},
	Short:   "SetExchangeFeeToSetter {from} {exchange} {feeToSetter} {password} {nonce}; Set exchange feeTo setter;",
	Example: `
	SetExchangeFeeToSetter UWDGLmQMfEeF6Fh8CGztrSktnHVpCxLiheYw UWTfBGxDMZX19vjnacXVkP51min9EjhYq43W UWDGLmQMfEeF6Fh8CGztrSktnHVpCxLiheYw 123456
		OR
	SetExchangeFeeToSetter UWDGLmQMfEeF6Fh8CGztrSktnHVpCxLiheYw UWTfBGxDMZX19vjnacXVkP51min9EjhYq43W UWDGLmQMfEeF6Fh8CGztrSktnHVpCxLiheYw 123456 1
	`,
	Args: cobra.MinimumNArgs(3),
	Run:  SetExchangeFeeToSetter,
}

func SetExchangeFeeToSetter(cmd *cobra.Command, args []string) {
	var passwd []byte
	var err error
	if len(args) > 3 {
		passwd = []byte(args[3])
...
func parseSEFTSParams(args []string, nonce uint64) (*types.Transaction, error) {
	...
	tx, err := transaction.NewSetFeeToSetter(from, exchange, feeToSetter, nonce, "")
	if err != nil {
		return nil, err
	}
	return tx, nil
